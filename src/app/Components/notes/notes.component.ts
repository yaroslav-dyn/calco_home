    ...
    <div fxLayout.gt-sm='row' fxLayout.xs="column" class="notes">
      <div fxFlex.gt-sm="20" class="notes_group">
        ...
        <div class="notes_group__item" fxLayout='row' *ngFor="let group of noteGroup; index as g">
          ...
          <button [ngClass]="{'w100 group-element': true, 'active' : group === groupFilter }" mat-stroked-button
                  (click)="editGroup(g, {title: group})"> {{group}} </button>
...
export class NotesComponent implements OnInit {
  ...
  public editGroupMarker = false;
  public editNotesMarker = false;

  public noteLayout = 'column wrap';
  ...
  editGroup(index, title) {
    if (this.editGroupMarker) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.noteGroup[index] = result.title;
        }
        this.updateGroups(this.noteGroup);
      });
    } else {
