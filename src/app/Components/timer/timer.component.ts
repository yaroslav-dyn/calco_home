...
@Component({
  ...
  <div class="app_timer half-screen">
    <div>
      ...
      <countdown [ngClass]="{'app_timer__remind': timeRemind }" class="app_timer__display" #cd1  [config]="timerConfig" (event)="doneTimer($event)"></countdown>
...
export class TimerComponent implements OnInit {
  ...
  public timeRemind: boolean = false

  public seconds: number = 30;
  public minutes: number = 0;
  public timeStart: boolean = false;
  ...
  setSeconds() {
    let sumMin = (this.hours * 3600) + (this.minutes * 60) + this.seconds
    if(sumMin > 0) {
      this.timerConfig = {
        leftTime: sumMin,
        demand: true,
